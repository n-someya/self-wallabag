...
**課題**:
- 既存のTerraform設定では間違ったSupabaseホスト（`db.ggqgjhxtbwvdodanabgc.supabase.co`）が使用されていた
...
1. 接続テストで正しいホスト名を特定
   ```bash
   psql "postgresql://postgres.ggqgjhxtbwvdodanabgc:PASSWORD@aws-0-ap-northeast-1.pooler.supabase.com:5432/postgres?sslmode=require"
   ```
...
3. 環境変数を正しく設定
   - `POSTGRES_USER`: `postgres.ggqgjhxtbwvdodanabgc`（プロジェクトIDを含む完全な形式）
...
services:
  wallabag:
    ...
    environment:
      ...
      - POSTGRES_PORT=5432
      - POSTGRES_USER=postgres.ggqgjhxtbwvdodanabgc
      - POSTGRES_PASSWORD=Someone620#Hama
      - PGDATABASE=postgres
      ...
      - SYMFONY__ENV__DATABASE_USER=postgres.ggqgjhxtbwvdodanabgc
      - SYMFONY__ENV__DATABASE_PASSWORD=Someone620#Hama
...
gcloud artifacts repositories describe wallabag \
  --project=plucky-vault-184315 --location=us-central1
```

公式Wallabagイメージをタグ付けしてプッシュ：
```bash
docker tag wallabag/wallabag:latest \
  us-central1-docker.pkg.dev/plucky-vault-184315/wallabag/wallabag:latest
...
supabase_port             = 5432
supabase_db_password      = "Someone620#Hama"
...
# Container Image
container_image = "us-central1-docker.pkg.dev/plucky-vault-184315/wallabag/wallabag:latest"
```
...
デプロイ結果：
- サービスURL: **https://wallabag-gobijnh4fa-uc.a.run.app**
- リージョン: `us-central1`
- イメージ: `us-central1-docker.pkg.dev/plucky-vault-184315/wallabag/wallabag:latest`
...
```bash
# HTTPレスポンスコード確認
curl -s -o /dev/null -w "%{http_code}" https://wallabag-gobijnh4fa-uc.a.run.app
# 結果: 302 (正常なリダイレクト)

# ページタイトル確認
curl -sL https://wallabag-gobijnh4fa-uc.a.run.app | grep -o "<title>.*</title>"
...
gcloud run services logs read wallabag \
  --region=us-central1 --project=plucky-vault-184315 --limit=20
```
...
2025-12-06 06:54:25 GET 302 https://wallabag-gobijnh4fa-uc.a.run.app/
2025-12-06 06:54:33 GET 200 https://wallabag-gobijnh4fa-uc.a.run.app/login
```
...
2. **Cloud Run設定**
   ...
   - サービスアカウント: `wallabag@plucky-vault-184315.iam.gserviceaccount.com`
...
**エラー**:
```
psql: error: could not translate host name "db.ggqgjhxtbwvdodanabgc.supabase.co" to address: Name does not resolve
```
...
**エラー**:
```
psql: error: connection to server ... failed: FATAL: database "postgres.ggqgjhxtbwvdodanabgc" does not exist
```
...
**本番URL**: https://wallabag-gobijnh4fa-uc.a.run.app
